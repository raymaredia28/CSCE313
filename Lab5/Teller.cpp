...
void teller_thread(BankAccount* const acc, const int64_t _amount)
{
        // TODO #1: call perform_transaction() function for acc
  	      //Perform each transaction in one thread
...
void teller_threadsafe(BankAccount* const acc, const int64_t _amount)
{
	...
	//Transactionw but for safe threads
...
int main(int argc, char* argv[])
{
        ...
        for(const int64_t& t: trans_arr){
                //Creating a thread for pushing back using normal thread
                std::thread tellerThread(teller_thread, &B, t);
               
	       	//Moving for memory issues 
                t_vec.push_back(std::move(tellerThread));
        }
        ...
        for(const int64_t& t: trans_arr){
                //Making safe threads 
               	std::thread tellerSafe(teller_threadsafe, &C, t);
                
		//memory safe moving
                t_vec2.push_back(std::move(tellerSafe));
                cout << "Added stuff todo4" << endl;//DEBUG
